...
1. 打开网盘到 `3.软件资料 -> Linux-5.10.145 -> 3.5-Demo` ，把 `3.5-Demo` 目录下载到 U 盘（提示：这个目录有 2GB 左右，建议使用 USB 3.0 的 U 盘）。
2. 下载完成后，把 U 盘插入开发板。
3. 把 U 盘挂载到文件系统。
...
1. 把 `3.5-Demo` 里的 `libtvm_runtime.so` 复制到开发板，参考命令如下：

.. code-block:: shell

   cp ~/3.5-Demo/libtvm_runtime.so /usr/lib64/

**注意**：`libtvm_runtime.so` 需要放到 `/usr/lib64/` 目录下。

2. 解压 `3.5-Demo` 里的 `rzv_ai_apps_v520_bin.tar.gz` 到开发板，参考命令如下：

.. code-block:: shell

   tar zxf ~/3.5-Demo/rzv_ai_apps_v520_bin.tar.gz -C ~

3. 解压 `3.5-Demo` 里的 `rzv_ai_apps_v520_lib.tar.gz` ，参考命令如下：

.. code-block:: shell

   tar zxf ~/3.5-Demo/rzv_ai_apps_v520_lib.tar.gz -C ~

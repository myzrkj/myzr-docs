...
* **下载固件**
   ...
   * 打开网盘到 `3.软件资料 -> Linux-5.10.145` ，下载 `3.2-固件` 目录。
   * 打开网盘到 `1.通用资料` ，下载 `1.3-工具` 目录。

* **复制文件**

   * 把 `3.2-固件` 目录的所有文件复制到 U 盘。
...
* 加载工具
   ...
   * 加载 Flash 工具：把 `3.2-固件` 目录里的 `Flash_Writer_SCIF-MYZR_RZV2H_EK320.mot` 文件直接拖入到 Tera Term 软件窗口上。

   .. figure:: /image/MYZR-瑞萨系列/MYZR-RZV2H-EK320/TeraTerm.SendFile.FlashWriter.png
      :alt: TeraTerm.SendFile.FlashWriter.png

   * 等待 Flash 工具传输完成，传输过程中可以看到传输进度。

   .. figure:: /image/MYZR-瑞萨系列/MYZR-RZV2H-EK320/TeraTerm.Transmit.FlashWriter.png
      :alt: TeraTerm.Transmit.FlashWriter.png

* 切换传输速率

   * 在上一步的 Flash 工具传输完成后，在 Tera Term 上输入 `sup`

   .. figure:: /image/MYZR-瑞萨系列/MYZR-RZV2H-EK320/TeraTerm.FW.SUP.png
      :alt: TeraTerm.FW.SUP.png

   * 修改串口速率（加快后续文件传输）：在 Trea Term 的菜单栏上点击 `Setup -> Serial port...` ，修改串口波特率为 `921600`。

   .. figure:: /image/MYZR-瑞萨系列/MYZR-RZV2H-EK320/TeraTerm.Serial.912600.png
      :alt: TeraTerm.Serial.912600.png
...
/
   ...
   .. figure:: /image/MYZR-瑞萨系列/MYZR-RZV2H-EK320/TeraTerm.FW.BL2.CMD.png
      :alt: TeraTerm.FW.BL2.CMD.png

   * 传输 bl2 固件：把 `3.2-固件` 目录的 `bl2_bp_emmc-myzr-rzv2h-ek320-8g.srec` 文件直接拖入到 Tera Term 软件窗口上（这里根据拿到的开发板的内存容量选择对应的文件），并等待传输完成。

   .. figure:: /image/MYZR-瑞萨系列/MYZR-RZV2H-EK320/TeraTerm.Transmit.BL2.png
      :alt: TeraTerm.Transmit.BL2.png

   .. figure:: /image/MYZR-瑞萨系列/MYZR-RZV2H-EK320/TeraTerm.FW.BL2.Done.png
      :alt: TeraTerm.FW.BL2.Done.png
...
/
   ...
   .. figure:: /image/MYZR-瑞萨系列/MYZR-RZV2H-EK320/TeraTerm.FW.FIP.CMD.png
      :alt: TeraTerm.FW.FIP.CMD.png

   * 传输 fip 固件：把 `3.2-固件` 目录的 `fip-myzr-rzv2h-ek320-8g.srec` 文件直接拖入到 Tera Term 软件窗口上（这里根据拿到的开发板的内存容量选择对应的文件），并等待传输完成。

   .. figure:: /image/MYZR-瑞萨系列/MYZR-RZV2H-EK320/TeraTerm.Transmit.FIP.png
      :alt: TeraTerm.Transmit.FIP.png

   .. figure:: /image/MYZR-瑞萨系列/MYZR-RZV2H-EK320/TeraTerm.FW.FIP.Done.png
      :alt: TeraTerm.FW.FIP.Done.png
...
|  **提示**：如果出现 `No boot file defined`，则是因为旧的环境变量还没有清除，需要执行下面命令后并重启开发板。

.. code-block:: shell

   env default -a; saveenv

* **登录刷机系统**：输入 root 并按 Enter 即可

.. code-block:: shell

   root

* **执行刷机**：复制下面的命令到终端执行（建议一次复制一段）

.. code-block:: shell

   cat <<EOF | sfdisk /dev/mmcblk0
   /dev/mmcblk0p1 : size=960M, type=83
   ...
   RTFS_FILE=core-image-weston-myzr-rzv2h-ek320.rootfs.tar.gz
   tar xvf /media/${RTFS_FILE} -C /mnt/p2

   sync; umount /mnt/p1; umount /mnt/p2; umount /media
...
* 附图
   ...
   .. figure:: /image/MYZR-瑞萨系列/MYZR-RZV2H-EK320/Update.Done.Login.png
      :alt: Update.Done.Login.png
...
|  When we want to compile the source code or compile the source code modified by ourselves, the first step is to configure the corresponding cross-compilation tool. For this set of source code, we compile based on the SDK compiled by Renesas official yocto.
...
|  The SDK installation package is in the MYZR-RZ -> 03_SDK directory. We generally use **poky-glibc-x86_64-core-image-qt-aarch64-smarc-rzg2l-toolchain-3.1.17.sh** for installation.
|  We can create a dedicated directory for rz on the host to store related source code, compilation tools, and some things that will be used later. For example, the directory I created is: /home/myzr/my-work/renesas, and under this directory I created 4 subdirectories:
...
|  After placing the tool in directory 03, proceed to install the tool.
...
|  Check cross-compilation tool version and other information after source
...
|  After seeing the version information, the compilation tool is configured successfully, and then we can proceed to the source code compilation steps. It should be noted that the source configuration needs to be performed every time a terminal window is opened.
...
|  Later, when we develop and debug by ourselves, we can perform different compilation configurations according to the parameters.
...
|  When compiling the kernel for the first time, you need to install the corresponding libraries on the virtual machine ubuntu
...
|  The dtb file is compiled according to different models of the development board: myzr-rzg2l-dsi.dtb, myzr-rzg2l-dsi.dtb, myzr-rzg2ul-eth.dtb, myzr-rzg2ul-lcd.dtb

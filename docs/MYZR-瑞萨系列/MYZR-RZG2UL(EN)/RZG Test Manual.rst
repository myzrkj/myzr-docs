...
| The rzg2l and rzg2ul use the same set of kernel and uboot source codes. The rzg2l uses the `core-image-qt-myzr-rzg2l-Release.xxx.tar.bz2` system, and the rzg2ul uses the `core-image-bsp-myzr-rzg2ul-Release.xxx.tar.bz2` system.

**After receiving the development board, directly use the factory default image system to perform the following functional tests for verification. Proceed with subsequent development only after the development board works properly.**
...
| 【Uboot Version】: u-boot-Release.xxx.tar.bz2
...
| Note: A static IP address 192.168.137.81 is set for eth0.
...
3. Configure the GPIO47_2 pin to output high level:

.. code-block:: shell

   # echo 1 > /sys/class/gpio/P47_2/value

| When measuring the GPIO47_2 pin with a multimeter, a voltage of approximately 3.3V (high level) can be detected.

4. Configure the GPIO47_2 pin to output low level:

.. code-block:: shell

   # echo 0 > /sys/class/gpio/P47_2/value

| When measuring the GPIO47_2 pin with a multimeter, a voltage of approximately 0V (low level) can be detected.

5. Configure the GPIO47_2 pin as input, ground the pin, and read its value:
...
3. Configure the GPIO7_3 pin to output high level:

.. code-block:: shell

   # echo 1 > /sys/class/gpio/P7_3/value

| When measuring the GPIO7_3 pin with a multimeter, a voltage of approximately 3.3V (high level) can be detected.

4. Configure the GPIO7_3 pin to output low level:

.. code-block:: shell

   # echo 0 > /sys/class/gpio/P7_3/value

| When measuring the GPIO7_3 pin with a multimeter, a voltage of approximately 0V (low level) can be detected.

5. Configure the GPIO7_3 pin as input, ground the pin, and read its value:
...
1. Short-circuit pins P3-7 and P3-9, which are UART2_RXD and UART2_TXD.
...
| 【Test Description】: Test using UART self-transmit and self-receive mode
| 【Interface Identification】: RS232
| 【System Devices】:
  - MYZR-RZG2L-MB200: /dev/ttySC0 (debug UART), /dev/ttySC1, /dev/ttySC3
  - MYZR-RZG2UL-MB200-ETH: /dev/ttySC0 (debug UART), /dev/ttySC1, /dev/ttySC4
  - MYZR-RZG2UL-MB200-LCD: /dev/ttySC0 (debug UART), /dev/ttySC1, /dev/ttySC4

| 【Interface Silkscreen】:
  - MYZR-RZG2L-MB200: J10-11 (debug UART/RS232_RX0), J10-12 (debug UART/RS232_TX0), J10-14 (RS232_RX3), J10-15 (RS232_TX3), J10-19 (RS232_RX1), J10-20 (RS232_TX1)
  - MYZR-RZG2UL-MB200-ETH: J16 (RS232_0 (debug UART), RS232_1, RS232_4)
  - MYZR-RZG2UL-MB200-LCD: J16 (RS232_0 (debug UART), RS232_1, RS232_4)

| Note: The debug UART does not require testing.
...
2. Enter the following command to perform RS232 transmit-receive test:
...
| 【Test Description】: RS485 uses differential signals and cannot be tested via self-transmit and self-receive mode.
...
1. Use a 485-232 converter to connect pins J16 (A, B) to the PC's USB-to-serial cable
2. Open the serial port debugging tool, set the baud rate to 9600, no parity bit, 8 data bits, and 1 stop bit
...
| You can see that the serial port debugging tool receives the string "123"

4. The development board receives data (send data from the PC side):

.. code-block:: shell

   # cat /dev/ttySTM2

| When the serial port debugging tool sends a string, the development board will receive the data:
...
| 【Test Description】: Send data between CAN ports
...
| Note: The MYZR-RZG2UL-MB200-LCD baseboard has only one CAN port, so self-transmit-receive testing is not possible. Two development boards can be used for CAN port interconnection testing.

**Test Operation**

| Take the MYZR-RZG2L-MB200 baseboard as an example:

1. Connect the CAN ports: connect CAN0_L to CAN1_L, and CAN0_H to CAN1_H
...
4. Send data via can1:

.. code-block:: shell

   # cansend can1 1F334455#1122334455667788

5. The following data will be received:
...
| 【Test Description】: Test using self-transmit and self-receive mode
| 【Interface Identification】: GPIO/SPI/UART
...
1. Short-circuit SPI1_MOSI and SPI1_MISO of P3, then enter the command:
...
1. Insert headphones with a microphone into the headphone jack of the development board
...
| 【Test Description】: After the WIFI is connected to the Access Point (AP), the development board sends ICMP packets to the external network to verify a normal connection.
...
.. code-block:: shell
   ...
   Format of the wpa_passphrase command: wpa_passphrase + WIFI name + WIFI password > /etc/wpa_supplicant.conf
...
| 【Test Description】: After the 4G connection is successfully established, the development board sends ICMP packets to the external network to verify a normal connection.
...
1. Power off the development board, connect the 4G module, attach the antenna, insert the SIM card, and then power on the evaluation board.
...
| Note: Only RZG2L supports DSI display function.

**Test Operation**

1. Power on the development board, press Enter to enter uboot command line mode before the uboot countdown ends, and input `printenv fdtfile` to check the current device tree name.
...
2. If the current device tree is not DSI, replace it:
...
4. Power off, connect the DSI display, and power on the development board to see the startup screen displayed on the monitor.
...
| 【Test Description】: The system uses RGB display by default. If HDMI display has been switched, it needs to be switched back.
...
| Note: Only MYZR-RZG2L-MB200 and MYZR-RZG2UL-MB200-LCD have LCD display function. The corresponding device tree for MYZR-RZG2UL-MB200-LCD is: myzr-rzg2ul-lcd.dtb, which does not need to be replaced.

**Test Operation**

| Replace RZG2L with RGB display:
...
4. Power off, connect the RGB display, and power on the development board to see the startup screen displayed on the monitor.
...
| 【Test Description】: Read the sampling output value of ADC pin.
| 【Interface Identification】: GPIO/SPI/UART
...
| Take MYZR-RZG2L-MB200 baseboard as an example:
...
| It can be seen that the ADC channel node is in the /sys/bus/iio/devices/iio:device0 directory.

2. Short the ADC_CH0 channel to pin J11:1 (i.e., short to ground), then read the ADC value.
...
3. The maximum sampling voltage must not exceed 1.8V, otherwise it may cause damage to the core board. The maximum sampling output value is 4096.
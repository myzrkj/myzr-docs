...
| There are multiple programming methods, and different images correspond to different programming methods. This document will introduce various image programming methods, and users can choose one or more methods to update the image according to the project conditions of development.

Programming with mytool
--------------------------
...
.. figure:: /image/MYZR-瑞萨系列/MYZR-RZFIVE/Mytool_1.png
   :alt: Mytool_1.png
...
.. figure:: /image/MYZR-瑞萨系列/MYZR-RZFIVE/Mytool_2.png
   :alt: Mytool_2.png

| Ensure that the development board is connected to the serial cable, select the COM port, and click the Connect button. (It is necessary to ensure that the COM port of the development board is not opened in other software.)

.. figure:: /image/MYZR-瑞萨系列/MYZR-RZFIVE/Mytool_3.png
   :alt: Mytool_3.png
...
.. figure:: /image/MYZR-瑞萨系列/MYZR-RZFIVE/Mytool_4.png
   :alt: Mytool_4.png

| Click the One-key operation button to start programming.
| Alternatively, you can skip the one-key operation button: first click the Loading mot button to load, then click Writing uboot to program. bl2 is the factory firmware and generally does not need to be updated. After programming is completed, click the Disconnect button to disconnect.

Programming with TF Card
--------------------------

**TF card programming can be used to update the kernel, device tree, and file system. This method enables comprehensive image update, and the subsequent operation is simple, so it is used for programming in the production stage.**

| **TF card requirements:** Ensure that the TF card has one Partition 1, and the memory of Partition 1 is larger than the total memory of all programming images.
...
**For the file system name, the version number needs to be removed, that is, rename it to (core-image-qt-myzr-rzg2l.tar.bz2).**
...
**This method can update the kernel and device tree images independently, and is suitable for the development and debugging stage.**
...
2. Enter the mount directory /mnt and delete the original image.
3. Then copy the new image to this directory.
4. Use the command to unmount and restart.

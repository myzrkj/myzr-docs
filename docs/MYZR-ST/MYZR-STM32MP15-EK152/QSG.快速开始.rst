...
2. 开发环境建立好之后，应参照 :doc:`《编译手册》 <./MYZR-STM32MP15-EK152编译参考手册>` 进行一次编译，并保留编译得到的目标文件。
3. 编译好目标文件之后，更新目标文件到设备，并最好参照 :doc:`《测试手册》 <./MYZR-STM32MP15测试手册>` 再进行一次测试，以验证编译出来的目标文件没有问题。
...
1. 首先，硬件工程师有必要了解我们开发板底板的介绍和基本原理，具体见 :doc:`《底板硬件介绍》 <./MYZR-STM32MP15.MB152.硬件手册>` 。
2. 硬件工程师打开网盘，下载硬件原理图文件进行参考，或基于我们的原理图进行设计。
3. 如果某些接口和功能需要变更，可以参考 :doc:`《核心板硬件介绍》 <./MYZR-STM32MP15.CB152.硬件手册>` 里的 **管脚定义&详细功能说明**。

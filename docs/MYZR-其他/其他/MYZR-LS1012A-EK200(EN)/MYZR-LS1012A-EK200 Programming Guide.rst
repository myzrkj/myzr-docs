MYZR-LS1012A-EK200 Programming Guide
======================================

Installing and Configuring the Programming Tool
-------------------------------------------------

**Install the Programming Tool**

|   Open the tool directory, double-click `CW_ARMv8_v2019.01_b190130_Win_Offline.exe` to start the installation. Use the default options for the installation path and configuration.

**Copy RCW and U-Boot Images to the Programming Tool Directory**

|   Copy `rcw_800.bin.swapped` and `ls1012a-ek200-uboot.bin` to the directory `C:\Freescale\CW4NET_v2019.01\CW_ARMv8\ARMv8\gdb\bin`.

**Configure the Programming Tool Script File**

1. Open the file `C:\Freescale\CW4NET_v2019.01\CW_ARMv8\ARMv8\gdb_extensions\flash\cwflash.py`
2. Modify the content in the file as follows: 
...
Programming RCW and U-Boot Images to the Development Board
-------------------------------------------------------------

**Put the Development Board into Download Mode**

|   Locate the 2-position DIP switch indicated by **BOOTMODE** or **BOOT SWITCH** on the development board. Set switch 1 to **ON** and switch 2 to **OFF**.

**Connect the Development Board to the Computer**

1. Turn on the power, plug the K20-JTAG module into the development board, and connect the other end to the computer via a MINI USB cable.
2. Double-click `mbedWinSerial_16466.exe` in the tool directory to install the driver.
3. After successful installation, open the computer's **Device Manager** to see the corresponding port as shown in the figure below.

.. figure:: /image/MYZR-其他/MYZR-LS1012A-EK200/LS1012-com.png
   :alt: LS1012-com.png

**Program RCW and U-Boot Images to the Development Board**

1. Double-click `"C:\Freescale\CW4NET_v2019.01\CW_ARMv8\ARMv8\gdb\bin\aarch64-fsl-gdb.bat"`.
2. A command window will pop up. Enter the command: `source ../../gdb_extensions/flash/cwflash.py`
...
4. After programming is complete, power off the development board, disconnect the USB cable, and set the DIP switch to boot mode: set switch 1 to **OFF** and switch 2 to **ON**.


Programming PPA, U-Image, DTB, and File System to the Development Board
--------------------------------------------------------------------------

**Download and Use the TFTPD Software**

1. Download the `tftpd64` software from the internet.
2. Open the software and click **Browse** to select the directory where the image files are located.

`Note: The directory must contain the following files: u-boot.bin, ppa.itb, rcw_800.bin.swapped`
`firmware-4.4.98.tar.bz2, kernel-4.4.98.tar.bz2, modules-4.4.98.tar.bz2, ramsys.itb, rootfs_jethro.tar.bz2, rootfs_jethro_config.tar.bz2, rootfs_jethro_update.tar.bz2, and system_recovery.sh`

**Program the PPA**

1. Connect the computer to the development board's network port using an Ethernet cable. Set the computer's Ethernet IP to `192.168.137.1`.
2. Restart the development board and enter the U-Boot command line.
...
4. After the ping is successful, enter the following commands:
...
Updating the Kernel, Device Tree, Modules, and File System
-------------------------------------------------------------

**TFTP Update Method**

1. Load the ramdisk system to start
...
2. After the development board starts up completely, enter the following commands:
...
2. After the development board finishes booting, enter the commands:
...
**Update Image, dtb and modules separately**
...
3. Check if the system automatically mounts the partition
...
**System automatically mounts partitions**

1. Copy the corresponding files to the /run/media/mmcblk0p1/ directory and replace the original files.
...
2. Unzip and update kernel modules
...
**If the system does not automatically mount the partition**

1. Check the fat partition address
...
3. Copy the corresponding files to the /mnt directory and replace the original files.
...
4. Unzip and update kernel modules

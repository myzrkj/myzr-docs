...
1. 将USB设备插入底板的USB接口,系统会输出如下类似信息:
...
2. 将USB设备从底板USB接口拔出,系统会输出如下类似信息:
...
测试说明:采用开发板向PC发送ICM P报文的方式进行测试

测试操作:

1. 配置电脑有线网卡IP为192.168.137.99。
...
3. 默认自动获取IP地址,但测试是配置静态ip来测,具体配置指令如下:
...
测试说明:WIFI连接到AP后,开发板向外网发送ICM P报文来验证连接正常

测试操作:

1. 把WIFI天线连接到U17接口上。

2. 输入如下指令,生成SSID的WPAPSK文件。

.. code-block:: shell

   ### wpa_passphrase 命 wpa_passphrase + wifi 名  + wifi 密  > 令 格 式 : 称 码
...
1. 关闭开发板电源,通过软排线连接MIPI屏和底板上的MIPI_DSI接口,重新启动开发板。

2. 启动开发板后,可以看到MIPI屏显示启动log和系统界面。
...
可以看到RS232串口终端输出MYZR,然后进入接收模式。

4. 在RS485串口终端,直接输入123(不显示),板子输出结果:
...
4. 终端输入如下命令使CAN接口在后台接收,并串口终端输入指令使CAN接口发送测试数据:

Core Board Hardware Manual
============================

Core Board View
-----------------

.. figure:: /image/MYZR-瑞芯微系列/MYZR-RK3588-EK314/RK3588的正面背面图片/MYZR-RK3588-CB314-font.jpg
   :alt: MYZR-RK3588-CB314-font

.. figure:: /image/MYZR-瑞芯微系列/MYZR-RK3588-EK314/RK3588的正面背面图片/MYZR-RK3588-CB314-back.jpg
   :alt: MYZR-RK3588-CB314-back   


Dimensions
-------------

|  82mm*60mm

.. figure:: /image/MYZR-瑞芯微系列/MYZR-RK3588-EK314/RK3588-CB314-size.png
   :alt: RK3588-CB314-size

Operating Temperature
------------------------

-  Commercial Grade:

   0°C ~ 70°C

-  Industrial Grade:

   -40°C ~ 85°C

Power Supply
--------------

-  12V Input

Operating System Support
--------------------------

-  Linux

Hardware Interfaces
----------------------

+-----------------------------+---------------------------------+-------------+-----------------------------------------------+
|       Interface Type        |     Interface Specification     | Max Cfg Qty |                  Description                  |
+=============================+=================================+=============+===============================================+
| Communication Interfaces    | Ethernet                        | 2           | IEEE 1588v2 hardware, MII/RMII/RGMII          |
+                             +---------------------------------+-------------+-----------------------------------------------+
|                             | USB2.0                          | 2           | 2 Channels of USB2.0                          |
+                             +---------------------------------+-------------+-----------------------------------------------+
|                             | UART                            | 8           | 4 x UART + 4 x USART (12.5 Mbit/s,            |
+                             +                                 +             +                                               +
|                             |                                 |             | ISO7816 interface, LIN, IrDA, SPI slave)      |
+                             +---------------------------------+-------------+-----------------------------------------------+
|                             | I2C                             | 5           | 5 Channels of I2C, supporting up to 400 kbps  |
+                             +---------------------------------+-------------+-----------------------------------------------+
|                             | SPI                             | 5           | 5 Channels of SPI, up to 50 Mbps each         |
+                             +---------------------------------+-------------+-----------------------------------------------+
|                             | CAN                             | 2           | 2 Channels of CAN, supporting CAN FD protocol |
+-----------------------------+---------------------------------+-------------+-----------------------------------------------+
| External Storage Interfaces | SDMMC                           | 2           | 2 × SDMMC up to 8 bits (SD/e•MMC™/SDIO)       |
+                             +---------------------------------+-------------+-----------------------------------------------+
|                             | DRAM                            | 1           | DDR3L, 16-bit data, 512M                      |
+                             +---------------------------------+-------------+-----------------------------------------------+
|                             | QSPI                            | 1           |                                               |
+-----------------------------+---------------------------------+-------------+-----------------------------------------------+
| Multimedia                  | LCD                             | 1           | LCD-TFT controller, up to 24-bit // RGB888    |
+                             +                                 +             +                                               +
|                             |                                 |             | up to WXGA (1366 × 768) @60 fps or up to      |
+                             +                                 +             +                                               +
|                             |                                 |             | Full HD (1920 x 1080) @ 30 fps                |
+                             +---------------------------------+-------------+-----------------------------------------------+
|                             | I2S                             | 4           |                                               |
+                             +---------------------------------+-------------+-----------------------------------------------+
|                             | SAI                             | 2           |                                               |
+-----------------------------+---------------------------------+-------------+-----------------------------------------------+


Pin Definition & Detailed Function Description
-------------------------------------------------

.. include:: HM.CB314-PinMux.rst

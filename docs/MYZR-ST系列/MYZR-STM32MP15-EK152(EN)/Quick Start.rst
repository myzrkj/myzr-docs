...
|   Testing Manual: :doc:`《Linux-5.4.1》 <./MYZR-STM32MP15 Test Manual>`
...
|   Compilation Manual: :doc:`《Linux-5.4.1》 <./MYZR-STM32MP15-EK152 Compilation Reference Manual>`
|   Driver and Device Manual: :doc:`《Linux-5.4.1》 <./MYZR-STM32MP15-EK152 Driver and Device Manual>`
...
2. After installing the terminal software, open the :doc:`《Startup Manual》 <./MYZR-STM32MP15-EK152 Startup Manual>` and follow the document to start the development board.
3. Once the development board starts successfully, perform a flashing operation by following the :doc:`《Flashing Guide Manual》 <./MYZR-STM32MP15CubeProgrammer Programming Guide>`. The purpose is to familiarize yourself with the flashing operation and prepare for the subsequent function verification.
4. After completing the flashing of the development board, open the corresponding :doc:`《Testing Manual》 <./MYZR-STM32MP15 Test Manual>` and conduct a round of tests to verify that all functions of the development board are normal.
...
2. After the development environment is set up, perform a compilation by following the :doc:`《Compilation Manual》 <./MYZR-STM32MP15-EK152 Compilation Reference Manual>` and retain the target files obtained from the compilation.
3. After compiling the target files, update the target files to the device. It is recommended to conduct another test by following the :doc:`《Testing Manual》 <./MYZR-STM32MP15 Test Manual>` to verify that the compiled target files are error-free.
...
1. First, it is necessary for hardware engineers to understand the introduction and basic principles of the base board of our development board. For details, refer to the :doc:`《Base Board Hardware Introduction》 <./MYZR-STM32MP15-EK152 Backplane Hardware Manual>`.
...
3. If changes to certain interfaces or functions are required, you can refer to the **Pin Definition & Detailed Function Description** in the :doc:`《Core Board Hardware Introduction》 <./MYZR-STM32MP15-EK152 Core Board Hardware Manual>`.
